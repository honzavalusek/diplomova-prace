...
```python
from src.features import Wav2Vec2Extractor

extractor = Wav2Vec2Extractor(model_name="facebook/wav2vec2-xls-r-300m")
embeddings = extractor.extract(audio, sr=16000)
# Output shape: (sequence_length, 1024)
```
...
extractor = Wav2Vec2Extractor(model_name="facebook/wav2vec2-xls-r-1b", device="cuda")
embeddings = extractor.extract(audio, sr=16000)
...
extractor = Wav2Vec2Extractor(model_name="facebook/wav2vec2-xls-r-2b", device="cuda")
embeddings = extractor.extract(audio, sr=16000)
...
```python
from src.features import XLSR53FeatureExtractor, Wav2Vec2Extractor
